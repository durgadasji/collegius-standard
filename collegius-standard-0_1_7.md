...
This standard specifies the structural conditions a self-curating body of domain practitioners must meet to function as credibly neutral, funder-independent distribution infrastructure. Adopting it requires combination: contributors who form or join a Collegius give up individual funding relationships in favor of a collective structural relationship with funders. The distribution invariant routes value to whoever currently satisfies the membership criterion. Individual positioning is replaced by collective infrastructure.

That trade is the standard's core premise. It is worth making because the structural form it produces is more durable than any individual grant relationship: funders commit to a domain rather than a project, the accountability is constitutive of the mechanism rather than a separate reporting burden, and the infrastructure persists through personnel changes and political moments that would end a project-based relationship.

Protocol Guild arrived at this configuration independently, through practical necessity, while building the funding infrastructure that Ethereum L1 core contributors needed. They are the reference case for what a Collegius produces when all structural conditions are present. The Collegius Standard makes that configuration explicit so any domain can adopt it deliberately.
...
| Version | Date | Notes |
|---------|------|-------|
| v0.1.7 | 2026-04-18 | Preamble added: names the combination requirement directly, the trade (flexibility for durability), and Protocol Guild as the reference case for what the structural form produces. |
